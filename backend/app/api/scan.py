...
async def scan_container(
    ...
    """
    logger.info(
        f"scan_container called with container_id={sanitize_log_message(str(container_id))}"
    )
    try:
        result = await ScanService.scan_container(db, container_id)
        logger.info(
            f"scan_container succeeded for container_id={sanitize_log_message(str(container_id))}"
        )
        return result
...
async def get_scan_results(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Container not found or scan results unavailable")
    except Exception as e:
