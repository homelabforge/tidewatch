...
class HttpServer(Base):
    ...
    # Server details
    name = Column(String, nullable=False, index=True)  # nginx, apache, caddy, granian, etc.
    ...
    detection_method = Column(String, nullable=False)  # labels, process, version_command, container_config

    # Dockerfile context (if detected from Dockerfile)
    dockerfile_path = Column(String, nullable=True)  # Path to Dockerfile containing the server
    ...
    ignored_version = Column(String, nullable=True)  # Which version transition was ignored (e.g., "2.6.0")
