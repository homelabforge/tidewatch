...
class Container(Base):
    ...
    current_digest = Column(String, nullable=True)  # e.g., "sha256:abc123..." for digest tracking
    ...
    policy = Column(String, default="manual", index=True)  # auto, manual, disabled, security - indexed for filtering
    scope = Column(String, default="patch")  # patch, minor, major
    include_prereleases = Column(Boolean, default=False)  # Include nightly, dev, alpha, beta, rc tags
    ...
    # Status
    update_available = Column(Boolean, default=False, index=True)  # Indexed for filtering updates
    latest_tag = Column(String, nullable=True)
    last_checked = Column(DateTime(timezone=True), nullable=True, index=True)  # Indexed for sorting
    last_updated = Column(DateTime(timezone=True), nullable=True)

    # Metadata
    labels = Column(JSON, default=dict)  # Docker labels from compose
    health_check_url = Column(String, nullable=True)
    health_check_method = Column(String, nullable=False, default="auto", server_default="auto")  # auto, http, docker
    health_check_auth = Column(String, nullable=True)
    release_source = Column(String, nullable=True)  # e.g., github:owner/repo or https://changelog

    # Intelligent restart configuration
    auto_restart_enabled = Column(Boolean, default=False)
    restart_policy = Column(String, default="manual")  # manual, on-failure, always, unless-stopped
    restart_max_attempts = Column(Integer, default=10)
    restart_backoff_strategy = Column(String, default="exponential")  # exponential, linear, fixed
    restart_success_window = Column(Integer, default=300)  # Seconds (5 minutes)

    # Update window configuration (time-based restrictions)
    update_window = Column(String, nullable=True)  # Format: "HH:MM-HH:MM" or "Days:HH:MM-HH:MM"

    # Dependency tracking for ordered updates
    dependencies = Column(String, nullable=True)  # JSON array of container names this depends on
    dependents = Column(String, nullable=True)  # JSON array of container names that depend on this
