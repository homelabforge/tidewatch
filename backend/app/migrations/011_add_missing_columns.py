    ...
    try:
        async with engine.begin() as conn:
            ...
            if not await column_exists(conn, "update_history", "backup_path"):
                ...
                await conn.execute(text("ALTER TABLE update_history ADD COLUMN backup_path TEXT"))
            ...
            if not await column_exists(conn, "containers", "current_digest"):
                ...
                await conn.execute(text("ALTER TABLE containers ADD COLUMN current_digest TEXT"))
                logger.info("  ✓ Column added")
            else:
            ...
            if not await column_exists(conn, "containers", "update_window"):
                ...
                await conn.execute(text("ALTER TABLE containers ADD COLUMN update_window TEXT"))
                logger.info("  ✓ Column added")
            else:
            ...
            if not await column_exists(conn, "containers", "dependencies"):
                ...
                await conn.execute(text("ALTER TABLE containers ADD COLUMN dependencies TEXT"))
                logger.info("  ✓ Column added")
            else:
            ...
            if not await column_exists(conn, "containers", "dependents"):
                ...
                await conn.execute(text("ALTER TABLE containers ADD COLUMN dependents TEXT"))
                logger.info("  ✓ Column added")
            else:
            ...
            if not await column_exists(conn, "updates", "recommendation"):
                ...
                await conn.execute(text("ALTER TABLE updates ADD COLUMN recommendation TEXT"))
                logger.info("  ✓ Column added")
            else:
            ...
            if not await column_exists(conn, "updates", "next_retry_at"):
                ...
                await conn.execute(text("ALTER TABLE updates ADD COLUMN next_retry_at TIMESTAMP"))
                logger.info("  ✓ Column added")
            else:
            ...
            if not await column_exists(conn, "updates", "last_error"):
                ...
                await conn.execute(text("ALTER TABLE updates ADD COLUMN last_error TEXT"))
                logger.info("  ✓ Column added")
            else:
