...
class CleanupService:
                ...
                try:
                    ...
                    if "image" in resource_type:
                        result["images"] = {
                            ...
                            "reclaimable": CleanupService._parse_bytes(
                                data.get("Reclaimable", "0B")
                            ),
                        }
    ...
    async def get_cleanup_preview(
        ...
        if mode in ["moderate", "aggressive"]:
            result["exited_containers"] = await CleanupService.get_exited_containers(
                exclude_patterns
            )
    ...
    async def prune_dangling_images() -> dict[str, Any]:
        ...
        try:
            ...
            if process.returncode != 0:
                error_msg = stderr.decode().strip()
                logger.error(
                    f"Failed to prune dangling images: {sanitize_log_message(str(error_msg))}"
                )
                return {
                    "success": False,
                    "error": error_msg,
            ...
            logger.info(
                f"Pruned {sanitize_log_message(str(images_removed))} dangling images, reclaimed {sanitize_log_message(str(CleanupService._format_bytes(space_reclaimed)))}"
            )
            ...
            for container in containers:
                try:
                    ...
                    if process.returncode == 0:
                        removed += 1
                        logger.debug(
                            f"Removed container: {sanitize_log_message(str(container['name']))}"
                        )
                    else:
    ...
    async def cleanup_old_images(
        ...
        if exclude_patterns:
            ...
            for image in old_images:
                try:
                    ...
                    if process.returncode == 0:
                        ...
                        logger.debug(
                            f"Removed old image: {sanitize_log_message(str(image.get('repository', image['id'])))}"
                        )
                    else:
                        # Image might be in use, skip silently
                        stderr_text = stderr_out.decode().strip()
                        if (
                            "image is being used" not in stderr_text
                            and "image has dependent" not in stderr_text
                        ):
            ...
            logger.info(
                f"Removed {sanitize_log_message(str(removed))} old images, reclaimed {sanitize_log_message(str(CleanupService._format_bytes(space_reclaimed)))}"
            )
        ...
        try:
            ...
            logger.info(
                f"Pruned {sanitize_log_message(str(images_removed))} old images (>{sanitize_log_message(str(days))} days), reclaimed {sanitize_log_message(str(CleanupService._format_bytes(space_reclaimed)))}"
            )
