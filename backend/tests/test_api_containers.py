...
@pytest.mark.asyncio
async def test_list_containers_with_data(
...
@pytest.mark.asyncio
async def test_update_container_policy(
    authenticated_client, db, sample_container_data, make_container
):
    ...
    response = await authenticated_client.put(
        f"/api/v1/containers/{container.id}/policy", json={"policy": "auto"}
    )
...
class TestContainerFilteringEndpoint:
    ...
    @pytest.mark.skip(
        reason="Status filtering requires Docker integration - containers don't have status in DB"
    )
    ...
    @pytest.mark.skip(
        reason="Label filtering requires Docker client mocking - labels not stored in DB"
    )
...
class TestContainerPolicyManagement:
    ...
    async def test_update_policy_to_disabled(
        ...
        response = await authenticated_client.put(
            f"/api/v1/containers/{container.id}/policy", json={"policy": "disabled"}
        )
...
class TestContainerStats:
    ...
    async def test_get_container_uptime(
        self, authenticated_client, db, make_container, mock_docker_client
    ):
    ...
    async def test_get_container_restart_count(
        self, authenticated_client, db, make_container, mock_docker_client
    ):
    ...
    async def test_get_container_history_pagination(
        ...
        response = await authenticated_client.get(
            f"/api/v1/containers/{container.id}/history?limit=2"
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert len(data) == 2
        ...
        # Test skip parameter
        response = await authenticated_client.get(
            f"/api/v1/containers/{container.id}/history?skip=2&limit=2"
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert len(data) == 2
...
class TestContainerSync:
    ...
    async def test_sync_removes_deleted_containers(
    ...
    async def test_sync_adds_new_containers(
        ...
        mock_docker_client.add_container(
            id="abc123", name="new-container", image="postgres:14", status="running"
        )
    ...
    async def test_sync_updates_container_status(
        self, authenticated_client, db, mock_docker_client, make_container
    ):
