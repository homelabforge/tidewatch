...
class TestListBackupsEndpoint:
    ...
    async def test_list_backups_empty(self, authenticated_client):
        ...
        with patch('app.api.backup.get_backup_files', return_value=[]), \
    ...
    async def test_list_backups_with_files(self, authenticated_client):
        ...
        with patch('app.api.backup.get_backup_files', return_value=mock_backups), \
             patch('app.api.backup.Path') as mock_path:
...
class TestRestoreBackupEndpoint:
    ...
    async def test_restore_valid_backup(self, authenticated_client, db):
        ...
        backup_data = {
            ...
            "settings": [
                {"key": "auto_update_enabled", "value": "true", "category": "general", "description": "Enable auto updates", "encrypted": False},
                {"key": "check_interval", "value": "120", "category": "general", "description": "Check interval", "encrypted": False}
            ]
        }
    ...
    async def test_restore_creates_safety_backup(self, authenticated_client, db):
        ...
        backup_data = {
            "version": "1.0",
            "settings": [
                {"key": "new_setting", "value": "new_value", "category": "general", "description": "", "encrypted": False}
            ]
        }
...
class TestBackupStatsEndpoint:
    ...
    async def test_stats_returns_database_info(self, authenticated_client):
        ...
        with patch('app.api.backup.get_database_stats') as mock_db_stats, \
    ...
    async def test_stats_returns_backup_count(self, authenticated_client):
        ...
        # Mock backups
        mock_backups = [
            {"filename": "backup1.json", "size_bytes": 10240, "created": "2025-01-01T12:00:00", "is_safety": False},
            {"filename": "backup2.json", "size_bytes": 20480, "created": "2025-01-02T12:00:00", "is_safety": False}
        ]
