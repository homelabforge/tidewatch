...
### Fixed
- My Projects no longer appear as stale containers
- "Scan Dep" no longer errors with a concurrent database-session error
...
### Changed
- Release-smoke harness parameterizes image tag and pre-builds with `docker rmi` for hermeticity

### Security
- OIDC callback sanitizes a known-safe log argument to silence CodeQL FP (alert #697)
...
### Fixed
- Host path validation failing with "Path traversal detected" when compose paths don't exist inside the container (#35)
...
- Settings tab deep-linking via `?tab=` query parameter (e.g. `/settings?tab=docker`)
- Typed `ContainerSyncResponse` schema for the `/containers/sync` endpoint

### Fixed
- Users with Dockge-style layouts (compose files in subfolders) no longer see an empty dashboard (#32)
